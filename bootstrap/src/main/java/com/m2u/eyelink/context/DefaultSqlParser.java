...
public class DefaultSqlParser implements SqlParser {
	...
	public NormalizedSql normalizedSql(final String sql) {
		...
		for (int i = 0; i < length; i++) {
			...
			case '\'':
				...
				if (lookAhead1(sql, i) == '\'') {
					...
					for (; i < length; i++) {
						...
						appendSeparatorCheckOutputParam(parsedParameter,
								stateCh);
					}
					break;
				}
			...
			case '8':
			case '9':
				// http://www.h2database.com/html/grammar.html look at the state
				// machine more
				if (numberTokenStartEnable) {
					...
					tokenEnd: for (; i < length; i++) {
						...
						case 'e':
							...
							break;
						default:
							// should look at the token outside the loop - not
							// here
		...
		if (change) {
			...
			return new DefaultNormalizedSql(normalized.toString(),
					parsedParameterString);
		} else {
			...
			case '9':
				...
				tokenEnd: for (; i < length; i++) {
					...
					case NUMBER_REPLACE:
						int numberIndex = 0;
						try {
							numberIndex = Integer.parseInt(outputIndex
									.toString());
						...
						try {
							String replaceNumber = outputParams
									.get(numberIndex);
					...
					case SYMBOL_REPLACE:
						int symbolIndex = 0;
						try {
							symbolIndex = Integer.parseInt(outputIndex
									.toString());
						} catch (NumberFormatException e) {
							// just append for invalid parameters
							normalized.append(outputIndex.toString());
						...
						try {
							String replaceSymbol = outputParams
									.get(symbolIndex);
	...
	public String combineBindValues(String sql, List<String> bindValues) {
		if (sql == null || sql.isEmpty() || bindValues == null
				|| bindValues.isEmpty()) {
			return sql;
		}
			...
			if (inQuotes) {
				...
				if (ch == '/') {
					...
					if (!bindValueQueue.isEmpty()) {
						result.append('\'').append(bindValueQueue.poll())
								.append('\'');
					}
				} else {
					result.append(ch);
