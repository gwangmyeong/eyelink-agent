...
import com.m2u.eyelink.util.NumberUtils;
...
public class StandardHostValveInvokeInterceptor implements AroundInterceptor {
	...
	public StandardHostValveInvokeInterceptor(TraceContext traceContext,
		...
		TomcatConfig tomcatConfig = new TomcatConfig(
				traceContext.getProfilerConfig());
		...
		if (proxyIpHeader == null || proxyIpHeader.isEmpty()) {
			...
			final String tomcatRealIpEmptyValue = tomcatConfig
					.getTomcatRealIpEmptyValue();
			this.remoteAddressResolver = new RealIpHeaderResolver<HttpServletRequest>(
					proxyIpHeader, tomcatRealIpEmptyValue);
		}
		this.isTraceRequestParam = tomcatConfig.isTomcatTraceRequestParam();
		this.excludeProfileMethodFilter = tomcatConfig
	...
	public void before(Object target, Object[] args) {
		...
		try {
			...
			// TODO STATDISABLE this logic was added to disable statistics
			// tracing
			if (!trace.canSampled()) {
				return;
			}
	...
	public static class Bypass<T extends HttpServletRequest> implements
			RemoteAddressResolver<T> {
	...
	public static class RealIpHeaderResolver<T extends HttpServletRequest>
		...
		public RealIpHeaderResolver(String realIpHeaderName,
				String emptyHeaderValue) {
			if (realIpHeaderName == null) {
				throw new NullPointerException(
						"realIpHeaderName must not be null");
			}
		...
		public String resolve(T httpServletRequest) {
			final String realIp = httpServletRequest
					.getHeader(this.realIpHeaderName);
			...
			if (emptyHeaderValue != null
					&& emptyHeaderValue.equalsIgnoreCase(realIp)) {
				return httpServletRequest.getRemoteAddr();
			}
	...
	private Trace createTrace(Object target, Object[] args) {
		...
		// check sampling flag from client. If the flag is false, do not sample
		// this request.
		...
		if (!sampling) {
			...
			if (isDebug) {
				logger.debug(
						"remotecall sampling flag found. skip trace requestUrl:{}, remoteAddr:{}",
		...
		if (traceId != null) {
			// TODO Maybe we should decide to trace or not even if the sampling
			// flag is true to prevent too many requests are traced.
			...
			if (trace.canSampled()) {
				...
				if (isDebug) {
					logger.debug(
							"TraceID exist. continue trace. traceId:{}, requestUrl:{}, remoteAddr:{}",
			...
			if (trace.canSampled()) {
				...
				if (isDebug) {
					logger.debug(
							"TraceID not exist. start new trace. requestUrl:{}, remoteAddr:{}",
							request.getRequestURI(), request.getRemoteAddr());
				}
			} else {
				if (isDebug) {
					logger.debug(
							"TraceID not exist. camSampled is false. skip trace. requestUrl:{}, remoteAddr:{}",
							request.getRequestURI(), request.getRemoteAddr());
				}
			}
			return trace;
	...
	private void setTraceMetadata(final HttpServletRequest request,
			final Trace trace) {
	...
	private void recordParentInfo(SpanRecorder recorder,
			HttpServletRequest request) {
		String parentApplicationName = request
				.getHeader(Header.HTTP_PARENT_APPLICATION_NAME.toString());
		if (parentApplicationName != null) {
			final String host = request.getHeader(Header.HTTP_HOST.toString());
			if (host != null) {
				recorder.recordAcceptorHost(host);
			} else {
				recorder.recordAcceptorHost(NetworkUtils.getHostFromURL(request
						.getRequestURL().toString()));
			}
			final String type = request
					.getHeader(Header.HTTP_PARENT_APPLICATION_TYPE.toString());
			final short parentApplicationType = NumberUtils.parseShort(type,
		...
		try {
			...
			if (this.isTraceRequestParam) {
				...
				if (!excludeProfileMethodFilter.filter(request.getMethod())) {
					final String parameters = getRequestParameter(request, 64,
							512);
					if (parameters != null && parameters.length() > 0) {
						recorder.recordAttribute(AnnotationKey.HTTP_PARAM,
								parameters);
					}
				}
			}
	 ...
	 *
	 * @param request
	 * @return TraceId when it is possible to get a transactionId from Http
	 *         header. if not possible return null
	 */
	private TraceId populateTraceIdFromRequest(HttpServletRequest request) {

		String transactionId = request.getHeader(Header.HTTP_TRACE_ID
				.toString());
		if (transactionId != null) {

			long parentSpanID = NumberUtils.parseLong(
					request.getHeader(Header.HTTP_PARENT_SPAN_ID.toString()),
					SpanId.NULL);
			long spanID = NumberUtils.parseLong(
					request.getHeader(Header.HTTP_SPAN_ID.toString()),
					SpanId.NULL);
			short flags = NumberUtils.parseShort(
					request.getHeader(Header.HTTP_FLAGS.toString()), (short) 0);

			final TraceId id = traceContext.createTraceId(transactionId,
					parentSpanID, spanID, flags);
			if (isDebug) {
	...
	private boolean samplingEnable(HttpServletRequest request) {
		// optional value
		final String samplingFlag = request.getHeader(Header.HTTP_SAMPLED
				.toString());
		if (isDebug) {
	...
	private String getRequestParameter(HttpServletRequest request,
		...
		while (attrs.hasMoreElements()) {
			...
			if (value != null) {
				params.append(StringUtils.abbreviate(
						StringUtils.toString(value), eachLimit));
			}
		}
		return params.toString();
	}

	private void deleteTrace(Trace trace, Object target, Object[] args,
