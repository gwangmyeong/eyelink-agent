...
COLLECTOR_IDENTIFIER=eyelink-collector
IDENTIFIER=maven.eyelink.identifier=$COLLECTOR_IDENTIFIER
...
KEY_VERSION="elagent.collector.version"
KEY_PORT="elagent.collector.port"
...
function func_check_running_eyelink_collector
{
...
function func_start_eyelink_collector
{
		version=$( func_read_properties "$KEY_VERSION" )
        pid=`nohup mvn -f $COLLECTOR_DIR/pom.xml clean package tomcat7:run -D$IDENTIFIER -Dmaven.collector.version=$version > $LOGS_DIR/$LOG_FILE 2>&1 & echo $!`
        echo $pid > $PID_DIR/$PID_FILE

        echo "---$COLLECTOR_IDENTIFIER initialization started. pid=$pid.---"

        end_count=0
	check_running_eyelink_collector=$( func_check_running_eyelink_collector )
        while [ "$check_running_eyelink_collector" == "false" ]; do
        ...
        if [[ "$check_running_eyelink_collector" == "true" ]]; then
