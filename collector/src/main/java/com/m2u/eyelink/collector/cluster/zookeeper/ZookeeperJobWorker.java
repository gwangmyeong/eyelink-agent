...
import com.m2u.eyelink.collector.cluster.zookeeper.job.ZookeeperJob;
...
public class ZookeeperJobWorker implements Runnable {
    ...
    private final ELAgentServerRepository pinpointServerRepository = new ELAgentServerRepository();
    ...
    public void addELAgentServer(ELAgentServer pinpointServer) {
    ...
    public void removeELAgentServer(ELAgentServer pinpointServer) {
        String key = getKey(pinpointServer);
        synchronized (lock) {
    ...
    public void run() {
        ...
        // may lead to ELAgentServer leak when events are left unresolved
