...
//        boolean success = elasticSearchTemplate.asyncPut(APPLICATION_TRACE_INDEX, put);
        boolean success = elasticSearchTemplate.asyncPut(ElasticSearchUtils.generateIndexName(span.getAgentId()), TYPE_APPLICATION_TRACE_INDEX, span.getMap());
        
        if (!success) {
            elasticSearchTemplate.put(ElasticSearchUtils.generateIndexName(span.getAgentId()), TYPE_APPLICATION_TRACE_INDEX, span.getMap());
        }
    }
